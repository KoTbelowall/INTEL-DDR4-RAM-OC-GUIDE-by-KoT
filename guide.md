...
2) Be sure to install one or more fans so that they blow over the RAM chips (even bad fans are better than no fans at all; 140mm is preferred; buy an aio/custom loop if you're on the air cooling otherwise you'll probably suffer from bad ram temps=unstable; [example of temp reduction](https://media.discordapp.net/attachments/898074377407574018/1011084065409081355/unknown-114.png)).
...
1) On most modern 4 dimm boards, [the best slots for installing RAM are 2 and 4](https://media.discordapp.net/attachments/898074377407574018/951203012376936458/unknown.png?width=1440&height=599) (but you can always check the manual and sometimes see hints on the motherboard itself).
2) Proper installation of RAM sticks to each other; Usually people recommend placing a RAM stick with a better voltage scaling in the first slot, but this is not correct. The correct way is to look at tRTL after memory training.
Check how the memory is trained at different freqs (3200+; 2-3 reboots at the same frequency), then write down these values and swap the RAM sticks with each other.
